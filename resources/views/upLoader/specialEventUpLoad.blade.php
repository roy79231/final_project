...
@section('name')
    ...
    <div class="content">
        <h1>加分事件</h1>
        <form action="{{ route('specialEventUpLoader.store') }}" method="POST">
            @csrf
        ...
        @foreach ($postAchievement as $post)
            <div class=box>
                <form id="form{{ $post->id }}" action="{{ route('specialEventUpLoader.edit',$post) }}" method="POST">
                    @csrf
                    @method('patch')
                    <div>
                ...
                <form action="{{route('specialEventUpLoader.destroy',$post)}}" method="POST">
                    @csrf
                    @method('delete')
